...
4. 노래의 주제나 메시지 요약"""}
        ...
        # 오디오 재생을 위한 선택 박스
        line_options = [f"{i+1}번 줄: {line[0]}" for i, line in enumerate(lyrics) if line[0].strip()]
        selected_line = st.selectbox("들을 가사 줄을 선택하세요:", line_options)
        ...
        # 선택된 줄의 오디오 재생
        if st.button("🔊 선택한 줄 듣기"):
            selected_index = line_options.index(selected_line)
            selected_text = lyrics[selected_index][0]
            try:
        ...
        try:
            full_lyrics = " ".join([line[0] for line in lyrics if line[0].strip()])  # 빈 문자열 제거
            if full_lyrics:
                audio_response = client.audio.speech.create(
                    model="tts-1",
                    voice="alloy",
                ...
                st.warning("가사가 비어있어 음성을 생성할 수 없습니다.")
        except Exception as e:
